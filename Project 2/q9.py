...
for i in range(len_all):
    ...
    for j in range(1, len_tmp):
        if tmpline[j] not in movie2vertex:
            ...
            movie2actor[movie2vertex[tmpline[j]]].append(actor_id)

print("Generating actor to movies map ...")
for movie, actors in movie2actor.items():
    for actor in actors:
...
for actor in vertex2actor:
    ...
    for movie in actor2movie[actor]:
        if movie in movie2rating and len(movie2actor[movie]) > 5:
            weight = len(movie2actor[movie])
            sum_score += weight * movie2rating[movie]   # better authority
            num_acts += weight
    if num_acts > 0:
        actor2rating[actor] = sum_score / num_acts
...
for n_movie in new_movies:
    ...
    nums = 0
    for actor in cast:
        if actor in actor2rating and 0 <= actor2movies_num[actor] <= 50:
            weight = actor2movies_num[actor]
...
for movie in movie2actor:
    if movie in movie2rating:# and idx_m < 50000 and rnd.random() < 0.15: # random 5K samples
...
#     if len(movie2actor[key]) <= 1 or key not in vertex2rating:
#         num_ignored += 1
#         continue
